...
public class test {
	public static void main(String[] args) throws IOException,
		...
		broadcastTarget.setAddress(new UdpAddress("127.0.0.1/161"));
		broadcastTarget.setTimeout(5000); // 5s
		sendAsyncRequest(snmp, createGetNextPdu(), broadcastTarget);
		Thread.sleep(6000); // main thread wait 6s for the completion of
	...
	private static PDU createSetPdu() {
		...
		pdu.add(new VariableBinding(new OID("1.3.6.1.2.1.1.5.0"),
				new OctetString("sysname"))); // sysName
		return pdu;
	}
	...
	private static void sendAsyncRequest(Snmp snmp, PDU pdu,
			CommunityTarget target) throws IOException {
